...
final class CharSequenceReader extends Reader {
    ...
    public CharSequenceReader(CharSequence seq)  throws IOException{
        if (seq==null)
            throw  new IOException("WordWrap:seq==null");
            this.seq = seq;
            }
    ...
    public synchronized int read(CharBuffer target) throws IOException {
        if (target==null)
            throw  new IOException("WordWrap:target==null");
        checkOpen();
        if (!hasRemaining()) {
            return -1;
        }
    ...
    public synchronized long skip(long n) throws IOException {
        if (n < 0)
            throw new IOException("WordWrap:N <0");
        checkOpen();
    ...
    public synchronized void mark(int readAheadLimit) throws IOException {
        if (readAheadLimit < 0)
            throw new IOException("WordWrap:readAheadLimit <0");
        checkOpen();
        mark = pos;
        }
