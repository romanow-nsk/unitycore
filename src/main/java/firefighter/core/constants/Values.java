...
public class Values {
    public final static int ReleaseNumber=643;                  // номер сборки сервера
    ...
    public final static int AsteriskDelayInSec = 10;            // Период опроса почты
    ...
    public final static int QRCodeSize = 1000;
    public final static int FacilityTokenSize = 100;            // Длина токена для qr-кода
    ...
    public final static int MongoDBTypeProxy=2;
    public final static int FatalExceptionStackSize=20;         // Стек вызовов при исключении
    public final static User superUser  =new User(Values.UserSuperAdminType,"xxx","xxx","xxx","superadmin",Values.DebugTokenPass,"89230000000");


    public final static EntityIndexedFactory EntityFactofy = new EntityIndexedFactory() {
        public void init() {
            put(new TableItem("Метка GPS", GPSPoint.class));
            put(new TableItem("Адрес", Address.class));
            put(new TableItem("Артефакт", Artifact.class));
            put(new TableItem("Пользователь", User.class));
            //---------- Реализовано в памяти ----------------------------------------
            //put(new TableItem("Уведомление",FireNotification.class));
            put(new TableItem("Настройки", WorkSettings.class));
            put(new TableItem("Улица", Street.class));
            put(new TableItem("Нас.пункт", City.class));
            put(new TableItem("Ошибка", BugMessage.class));
            put(new TableItem("Персоналия", Person.class));
            put(new TableItem("Отчет", ReportFile.class));
            put(new TableItem("Состояние", ServerState.class));
            put(new TableItem("Подсказка", HelpFile.class));                     // Сборка 586
            put(new TableItem("Контакт", Contact.class,false));          // Сборка 623 - не таблица
            put(new TableItem("Аккаунт", Account.class,false));          // Сборка 623 - не таблица
            put(new TableItem("Спец.файла", FileNameExt.class,false));   // Сборка 623 - не таблица
            put(new TableItem("Дата/время", OwnDateTime.class,false));   // Сборка 623 - не таблица
            put(new TableItem("E-mail", Mail.class,false));              // Сборка 623 - не таблица
            put(new TableItem("Телефон", Phone.class,false));            // Сборка 623 - не таблица
            put(new TableItem("Телефоны", PhoneList.class,false));       // Сборка 623 - не таблица
            put(new TableItem("Сумма", MoneySum.class,false));           // Сборка 636
            put(new TableItem("Аккаунт", Account.class));                         // Сборка 637
            }
        };
    ...
    static  {
        ...
        PrefixMap.put("WorkSettings.lastReglamentMonth","m");       //636
        PrefixMap.put("WorkSettings.technicianModeStartTime","b");  //636
        PrefixMap.put("WorkSettings.technicianModeStopTime","e");   //636
    ...
    public final static int WSgpsOnWayPeriod = 2;           // Период GPS на пути к объекту (мин)
    public final static int WSgpsOnFacilityPeriod = 10;     // Период GPS на  объекте (мин)
    public final static int WSminDistanceToFacility = 200;  // Мин. расстояние до объекта для активации
    public final static int WSworkDayBegin = 10;            // Начало рабочего дня
    public final static int WSworkDayEnd = 18;              // Конец рабочего дня
    public final static int WSgpsValidDelay = 5;            // Интервал УСТАРЕВАНИЯ последних координат (мин)
